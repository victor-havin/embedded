...
using Broker = TestBroker<TimerEvent>;
using Subscription = TestBroker<TimerEvent>::Subscription;
using Subscriber = TestBroker<TimerEvent>::Subscriber;

class TimerEventBroker;
class ActiveSubscriber;
class CountingSubscriber;

//------------------------------------------------------------------------------
// Timer event broker
class TimerEventBroker: public Broker
{
public:
    TimerEventBroker():Broker(128, TimerEvent::LAST){}
    ~TimerEventBroker() {}
...
//------------------------------------------------------------------------------
class ActiveSubscriber : public Subscriber
{
public:
    ActiveSubscriber(TimerEventBroker* broker) : 
        Subscriber(static_cast<Broker*>(broker)) {}
    ...
    void begin() {
        subscribe(Subscription(
            ...
            this, 
            0, 
            [this](const Subscription& s, const TimerEvent& e){
                receive(e);
            }
        ));
    }
...
public:
    ...
    CountingSubscriber(TimerEventBroker* broker) :
        Subscriber(static_cast<Broker*>(broker)) {}
...
void test_timer_events()
{
    ...
    ActiveSubscriber active_subscriber(broker);
    CountingSubscriber counting_subscriber(broker);
    broker->publish(TimerEvent::SECONDS);
    broker->publish(TimerEvent::MILLISECONDS);
    active_subscriber.begin();
    counting_subscriber.begin();
    broker->begin();
    DELAY(timeout * 1000); 
    broker->end();
    std::cout << std::endl;
    unsigned start = counting_subscriber.get_start_time() ;
    unsigned finish = counting_subscriber.get_finish_time() ;
